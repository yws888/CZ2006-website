...
with open(os.path.join(os.getcwd(), "CZ2006", "utility","resale-flat-prices.csv"),  encoding="utf-8") as f:
    ...
    for row in reader:
        ...
        HDBResaleFlat.objects.create(monthOfSale = row[0], town = row[1], flatType = row[2], blockNo = row[3], streetName = row[4], storeyRange = row[5], floorArea= row[6], flatModel = row[7], leaseCommencementYear = row[8], remainingLease = templcY, resalePrice = row[10] )